...
class MultiLevelAttention(nn.Module):
    ...
    def __init__(self, encoder_dim, decoder_dim, attention_dim, retrieved_dim):
        ...
        self.cat_att = nn.Linear(
            retrieved_dim, attention_dim
        )  # linear layer to calculate values to be softmax-ed
        self.full_multiatt = nn.Linear(
            attention_dim, 1
        )  # linear layer to calculate values to be softmax-ed
    ...
    def forward(self, encoder_out, decoder_hidden, retrieved_target):
        ...
        att_h = self.decoder_att(decoder_hidden).unsqueeze(
            1
        )  # (batch_size, attention_dim)
        att = self.full_att(self.tanh(att_v + att_h)).squeeze(
            2
        )  # Eq.4 (batch_size, num_pixels)
        alpha = self.softmax(att)  # (batch_size, num_pixels)
        visual_context = (encoder_out * alpha.unsqueeze(2)).sum(
            dim=1
        )  # (batch_size, encoder_dim)

        visual_and_retrieved = torch.cat(
            ([visual_context.unsqueeze(1), retrieved_target.unsqueeze(1)]), dim=1
        )
        att_vr = self.cat_att(visual_and_retrieved)  # visual with retrieved target
        att_hat = self.full_multiatt(self.tanh(att_vr + att_h)).squeeze(
            2
...
class DynamicCore(nn.Module):
    def __init__(self, cfg):
        ...
        self.embed = nn.Sequential(
            nn.Linear(self.embed_input_dim, int(self.embed_input_dim / 2)),
            nn.ReLU(),
            nn.Dropout(self.drop_prob_lm),
            nn.Linear(int(self.embed_input_dim / 2), self.embed_dim),
            nn.ReLU(),
            nn.Dropout(self.drop_prob_lm),
        )

        self.module_att_lstm = nn.LSTMCell(
            self.embed_dim + cfg.model.speaker.rnn_size, cfg.model.speaker.rnn_size
        )

        self.weight_fc = nn.Sequential(
            nn.Linear(cfg.model.speaker.rnn_size, 3), nn.Softmax(dim=1)
        )

        self.pos1 = nn.Sequential(
            nn.Linear(512, 512), nn.ReLU(), nn.Dropout(self.drop_prob_lm)
        )
        self.softmax = nn.Softmax(dim=1)

        self.dropout = nn.Dropout(0.5)

        self.weight_pos = nn.Linear(cfg.model.speaker.rnn_size, 16)

        self.pos2 = nn.Linear(16, cfg.model.speaker.rnn_size)

        self.gate1x = nn.Sequential(
            nn.Linear(
                cfg.model.speaker.rnn_size * 2 + cfg.model.speaker.input_dim,
                cfg.model.speaker.rnn_size * 2 + cfg.model.speaker.input_dim,
            ),
            nn.ReLU(),
            nn.Dropout(self.drop_prob_lm),
        ...
        self.gate2x = nn.Linear(
            cfg.model.speaker.rnn_size * 2 + cfg.model.speaker.input_dim,
            cfg.model.speaker.input_dim,
        )

        self.lang_lstm = nn.LSTMCell(
            cfg.model.speaker.word_embed_size + cfg.model.speaker.input_dim,
            cfg.model.speaker.rnn_size,
        )

        # self.attention = MultiLevelAttention(encoder_dim, decoder_dim, attention_dim, retrieved_dim)
    ...
    def forward(
        ...
        h_mod_att, c_mod_att = self.module_att_lstm(
            module_att_lstm_input, (state[0][0], state[1][0])
        )
        module_weights = self.weight_fc(h_mod_att)
        self.module_weights = module_weights

        vpos = self.pos1(prev_h)
        dpos = self.weight_pos(vpos)
        dpos = self.dropout(dpos)
        pos_weights = self.softmax(dpos)
        ppos = self.pos2(pos_weights)

        feats = torch.cat(
            [
                loc_feat_bef.unsqueeze(1),
                feat_diff.unsqueeze(1),
                loc_feat_aft.unsqueeze(1),
            ],
            1,
        )
        weights_expand = module_weights.unsqueeze(2).expand_as(feats)
        att_feat = (feats * weights_expand).sum(1)  # (batch, feat_dim)
        gate_input = torch.cat([prev_h, ppos, att_feat], -1)
        gate = torch.sigmoid(self.gate2x(self.gate1x(gate_input)))

        gated_att_feat = gate * att_feat

        lang_lstm_input = torch.cat([xt, gated_att_feat], 1)
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], state[1][1]))

        output = F.dropout(h_lang, self.drop_prob_lm, self.training)
        state = (torch.stack([h_mod_att, h_lang]), torch.stack([c_mod_att, c_lang]))

        return output, state, dpos
...
class DynamicSpeaker(CaptionModel):
    def __init__(self, cfg, vocab_size=0):
        super().__init__()
        if vocab_size == 0:
        ...
        self.init_c = nn.Linear(cfg.model.speaker.embed_dim, self.rnn_size)
        self.module_weights = []

    def init_hidden(self, batch_size, retrieval_embed):
        ...
        hidden[1][1] = self.init_c(retrieval_embed)
        return hidden
    ...
    def _forward(
        ...
        state = self.init_hidden(batch_size, retrieval_embed)

        # outputs are logprobs
        outputs = feat_bef.new_zeros(batch_size, self.seq_length, self.vocab_size)
        outputs_pos = feat_bef.new_zeros(batch_size, self.seq_length, 16)
        ...
        for i in range(self.seq_length):
            if self.training and i >= 1 and self.ss_prob > 0.0:
                ...
                if sample_mask.sum() == 0:
                    ...
                    sample_ind = sample_mask.nonzero().view(
                        -1
                    )  # idx for sampling from batch
                    it = seq[:, i].data.clone()
                    # pos = pos_feat[:, i].data.clone()
                    prob_prev = torch.exp(
                        outputs[:, i - 1].detach()
                    )  # fetch previous distribution (N x (M+1))
                    it.index_copy_(
                        0,
                        sample_ind,
            ...
            output, state, output_pos = self.get_logprobs_state(
                it,
                feat_bef,
                feat_aft,
                feat_diff,
    ...
    def get_logprobs_state(
        ...
        output, state, output_pos = self.core(
            xt,
            feat_bef,
            feat_aft,
            feat_diff,
        ...
        return log_probs, state, log_pos_probs
    ...
    def _sample_beam(self, feat_bef, feat_aft, feat_diff, cfg={}):
        # start fresh
        self.module_weights = []
        ...
        for k in range(batch_size):
            ...
            logprobs, state = self.get_logprobs_state(
                it, tmp_feat_bef, tmp_feat_aft, tmp_feat_diff, state
            )
    ...
    def _sample(
        ...
        if beam_size > 1:
            return self._sample_beam(feat_bef, feat_aft, feat_diff, cfg)
        ...
        for t in range(self.seq_length + 1):
            ...
            # pos = pos_feat[:, t].clone()
            logprobs, state, log_pos_probs = self.get_logprobs_state(
                it,
                feat_bef,
                feat_aft,
                feat_diff,
            ...
            if sample_max:
                ...
                # find the highest probability of the sampled word
                it = torch.multinomial(prob_prev, 1)
                sample_logprobs = logprobs.gather(
                    1, it
