...
parser.add_argument(
    ...
    type=str,
    default="mode2",
    help="mode0: use image directly; mode2: use region features",
)
parser.add_argument(
    "--graph",
    type=str,
...
parser.add_argument("--eval_difference", action="store_true", default=False, help="")
parser.add_argument("--is_hpc", action="store_true", default=False, help="")
...
parser.add_argument(
    ...
    type=str,
    default="both",
    choices=["both", "location", "single_ana", "single_loc"],
)
...
parser.add_argument("--reaug_k", type=int, default=3)
parser.add_argument("--faiss_threshold", type=float, default=0.0)

parser.add_argument("--infonce", type=float, default=0.5)
parser.add_argument("--rag_state_label", type=float, default=1.0)
...
cfg.faiss.threshold = args.faiss_threshold

cfg.hpc.is_hpc = args.is_hpc
cfg.train.optim.lr = args.lr
cfg.faiss.reaug_k = args.reaug_k
...
exp_name = (
    exp_name
    + "_"
    + str(cfg.train.optim.lr)
    + "_"
    + "coef%f-%f" % (args.coef_sem, args.coef_spa)
    + "_"
...
annotation_file = (
    "data/mimic_gt_captions_test.json" if args.eval_target == "test" else "data/mimic_gt_captions_val.json"
)
...
while t < cfg.train.max_iter:
    ...
    if epoch > cfg.train.scheduled_sampling_start and cfg.train.scheduled_sampling_start >= 0:  # skip
        ...
        ss_prob = min(
            cfg.train.scheduled_sampling_increase_prob * frac,
    ...
    for i, batch in enumerate(train_loader):
        ...
        if args.setting == "mode2":
            ...
            q_sem_adj_matrix = process_matrix(
                q_sem_dj_matrix,
                cfg,
                sc_feats.shape[1],
                sc_feats.device,
        ...
        speaker_output_pos, speaker_output_pos_tag = speaker._forward(
            chg_pos_feat_bef,
            chg_pos_feat_aft,
            chg_pos_feat_diff,
            labels,
        ...
        infonce_loss = infonce_criterion(torch.cat((chg_pos_feat_diff, faiss_chg_pos_feat_diff), 0))
        infonce_loss = args.infonce * infonce_loss

        total_loss = speaker_loss + 2.5e-03 * att_sum + entropy_loss + infonce_loss
        ...
        if t % cfg.train.snapshot_interval == 0:
            ...
            checkpoint = {"change_detector_state": chg_det_state, "speaker_state": speaker_state, "model_cfg": cfg}
            save_path = os.path.join(snapshot_dir, snapshot_file_format % t)
            ...
            with torch.no_grad():
                ...
                for val_i, val_batch in enumerate(tqdm(val_loader)):
                    ...
                    if args.setting == "mode2":
                        ...
                        d_sem_adj_matrix = process_matrix(
                            d_sem_adj_matrix,
                            cfg,
                            d_feats.shape[1],
                            d_feats.device,
                        ...
                        q_sem_dj_matrix = process_matrix(
                            q_sem_dj_matrix,
                            cfg,
                            sc_feats.shape[1],
                            sc_feats.device,
                        ...
                        rag_d_adj_matrix = process_matrix(
                            rag_d_adj_matrix,
                            cfg,
                            d_feats.shape[1],
                            d_feats.device,
