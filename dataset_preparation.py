...
def save_h5(questions, answers, pos, label_start_idx, label_end_idx, feature_idx, max_seq=60, times=0, length=100):
    ...
    if times == 0:
        ...
        questions_dataset = h5f.create_dataset(
            "questions", (length, 20), maxshape=(None, 20), chunks=(100, 20), dtype="int64"
        )
        answers_dataset = h5f.create_dataset(
            "answers", (length, max_seq), maxshape=(None, max_seq), chunks=(100, max_seq), dtype="int64"
        )
        pos_dataset = h5f.create_dataset(
            "pos", (length, max_seq), maxshape=(None, max_seq), chunks=(100, max_seq), dtype="int64"
        )
        label_start_idx_dataset = h5f.create_dataset(
            "label_start_idx", (length, 1), maxshape=(None, 1), chunks=(100, 1), dtype="int64"
        )
        label_end_idx_dataset = h5f.create_dataset(
            "label_end_idx", (length, 1), maxshape=(None, 1), chunks=(100, 1), dtype="int64"
        )
        feature_idx_dataset = h5f.create_dataset(
            "feature_idx", (length, 2), maxshape=(None, 2), chunks=(100, 2), dtype="int64"
        )
    else:
    ...
    for i in tqdm(range(len(d))):
        ...
        for word in question_list + answer_list:
            if word not in vocab:
                ...
                warnings.warn(
                    "Unknown word: %s. Please be aware that this may result in the checkpoint we provided not functioning properly"
                    % word
                )
...
def main():
    ...
    parser.add_argument(
        ...
        "--transform_dataset",
        action="store_true",
        help="transform questions and answers into the format that can be used in the model",
    )
